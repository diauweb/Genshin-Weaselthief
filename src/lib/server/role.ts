...
import { tillCurrentOid, find, findIter, findOne } from "./db";
import { inlineLanguage } from "./util";
import { inlineNpc } from './dialog';
...
export async function getNpc (v : string) {
    ...
    return await inlineLanguage(result);
}
...
export async function getDetailNpc(v: string) {
    const npc = await inlineLanguage(await getNpc(v));
    ...
    return {
        ...
        dialogs: await Promise.all(dialogs.map(e => inlineLanguage(inlineNpc(e)))),
    }
}
