import { tillCurrentOid, findOne, collection } from './db';
import { inlineLanguage } from './util';
import { getNpc } from './role';

export async function inlineNpc(dialog) {
	const d = await dialog;
	if (d?.TalkRole__Type === 'TALK_ROLE_NPC') {
		d.TalkRoleNpc = await getNpc(dialog.TalkRole__Id);
	}
	return d;
}

export async function searchDialogContaining(mapId: string) {
	console.time(`search_dialog ${mapId}`);
	...
	console.timeEnd(`search_dialog ${mapId}`);
	return inlineNpc(rst);
}

export async function getDialog(id: string) {
	const dialog = await findOne('Dialog', {
		_ver: tillCurrentOid(),
		Id: parseInt(id)
	});

	return inlineNpc(dialog);
}
...
export async function searchReminder(id: string) {
	const result = await findOne('Reminder', {
		_ver: tillCurrentOid(),
		$or: [{ SpeakerTextMapHash: parseInt(id) }, { ContentTextMapHash: parseInt(id) }]
	});
...
export async function searchAllReminder(id: string) {
	...
	return await Promise.all(result.map(inlineLanguage));
}
